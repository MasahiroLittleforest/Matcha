class ProjectsController < ApplicationController
  ...
  def show
    ...
    @not_canceled_participants = User.joins(:applikations).where(applikations: { project_id: @project.id, cancel: false }).group(:id).merge(Applikation.order("applikations.updated_at ASC"))
    @canceled_participants = User.joins(:applikations).where(applikations: { project_id: @project.id, cancel: true }).group(:id).merge(Applikation.order("applikations.canceled_at ASC"))
    #@default_project_image_id = @project.project_category_id

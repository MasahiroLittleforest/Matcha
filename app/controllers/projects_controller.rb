class ProjectsController < ApplicationController
  ...
  def show
    ...
    @not_canceled_participants = User.joins(:applikations).includes(:applikations).where(applikations: { project_id: @project.id, cancel: false }).group(:id)
    @canceled_participants = User.joins(:applikations).includes(:applikations).where(applikations: { project_id: @project.id, cancel: true }).group(:id)
    #@default_project_image_id = @project.project_category_id
